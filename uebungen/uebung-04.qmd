...
__b.__ Erstellen ein _RNotebook_ names `uebung-04`.
...
__b.__ Lesen Sie den Datensatz `couples_extraversion.csv` aus dem `data` Ordner ein und speichern Sie ihn in die Variable `d_extraversion`. 
...
__e.__  Führen Sie einen t-Test mit der Funktion `t.test()` durch. Wie lautet Ihre Testentscheidung?
...
__f.__ Berechnen Sie die Effektgrösse mit der Funktion `cohen.d()` aus dem Package {effsize}. Wie gross ist der Effekt nach Cohen (1998)? 
...
## Aufgabe 4.4: Zusatzaufgaben

:::{.callout-tip appearance="default" title="Zusatzaufgaben"}

__Dies sind Zusatzaufgaben und müssen nicht gelöst werden.__

:::

__a.__ Diskutieren Sie folgende Fragen:
...
__b.__ Was geschieht, wenn Sie in _Aufgabe 4.4_ statt `paired = TRUE` `paired = FALSE` verwenden?
