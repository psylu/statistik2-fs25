...
> $b_1$: Regressionsgewicht, Regressionskoeffizient, Steigung (_slope_) für UV/Prädiktorvariable 1

> $b_2$: Regressionsgewicht, Regressionskoeffizient, Steigung (_slope_) für UV/Prädiktorvariable 2
...
> [Codebeispiel der Website](../datenanalyse/einfachelineareregression.html#voraussetzungen-überprüfen-1)
...
performance::check_heteroscedasticity(m_x1x2)
performance::check_normality(m_x1x2)
```
...
> R: multiple Korrelation --> R2: quadrierte multiple Korrelation (standardisierter Wert)
> dies kann gezeigt werden indem die beiden Werte für das einfache Modell berechnet werden, aus der nachfolgenden Tabelle, evtl. dort kurz zeigen:

```{r}
cor(d_sat_act$age, d_sat_act$ACT)
sqrt(0.01258) # square-root of R2 in model with age as a predictor on ACT
```
...
> 10 min.
> Wichtig, Zeit dafür am Schluss einplanen!

```{r}
summary(m_x1x2)
summary(m_x1x2)$coefficients
```
