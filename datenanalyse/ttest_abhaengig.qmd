...
| Abhängigkeit der Gruppen/Stichproben | Die Messwerte _zwischen_ den beiden Stichproben sind verbunden (abhängige Gruppen). | Bei unabhängigen Gruppen kann ein _t_-Test für unabhängige Stichproben verwendet werden (siehe [Kapitel: t-Test für abhängige Stichproben](ttest_unabhaengig.qmd)) |
...
Für den _t_-Test werden die Daten hier in einem `wide`-Format benötigt. Für die Visualisierung wird eine Datensatz im `long`-Format benötigt.
...
Falls der exakte _p_-Wert sehr klein ist, beispielsweise  $p=0.00000012$ bzw. kleiner als $0.001$, wird dies folgendermassen notiert: $p < .001$.^[https://apastyle.apa.org/instructional-aids/numbers-statistics-guide.pdf]

> Die kognitive Leistung von Personen vor und nach dem Treatment _X_ unterscheidet sich nicht signifikant, _t_(15) = 0.75, _p_ = .47.
...
::: {.callout-caution appearance="default" title="Hands-on: Beispieldatensatz Stimmungsexperiment vorverarbeiten" collapse="true"}
...
Für den _t_-Test werden die Daten in einem `long` Format benötigt. Daher müssen die Daten vom bisherigen `wide` Format in ein `long` Format umstrukturiert werden, Die Variable `id` wird mit `as.factor()` in einen Faktor konvertiert.
...
#### __5. Deskriptivstatisik und Visualisierung__
...
p_boxplot <- d_fear_long |> 
  ...
  scale_colour_manual(values = c("pre" = "red4",
                               "post" = "blue4")) +
  labs(title = "Effect of Social Media on Fear from the Future",
